...
import com.honstat.crawler.service.utils.CommonUtil;
 ...
 */
@Service
public class SystemInfoMonitorManager {

    private static final int CPUTIME = 5000;
    /**
     * 网管进程信息采集周期(注意：PERIOD_TIME 一定要大于 SLEEP_TIME )
     */
    private static int PERIOD_TIME = 1000 * 120;
    /**
    ...
    private String pid = "";
    private Timer sysInfoGetTimer = null;
    private Timer sysInfoGetTimerCustom = null;
    public Logger logger = Logger.getLogger(SystemInfoMonitorManager.class);
    @Autowired
    ...
    public void init() {
        ...
        sysInfoGetTimer = new Timer("sysInfoGet");
        sysInfoGetTimer.schedule(new SysInfoAcquirerTimerTask(), 10 * 1000, PERIOD_TIME);
    }

    public void resetTimerSpeed(Integer time) {
        try {
            if (sysInfoGetTimer != null) {
                sysInfoGetTimer.cancel();
                sysInfoGetTimer.purge();
            }
            if (sysInfoGetTimerCustom != null) {
                sysInfoGetTimerCustom.cancel();
                sysInfoGetTimerCustom.purge();
            }
            PERIOD_TIME = time;
            sysInfoGetTimerCustom = new Timer("sysInfoGetCustom");
            sysInfoGetTimerCustom.schedule(new SysInfoAcquirerTimerTask(), 10 * 1000, PERIOD_TIME);
            logger.info("定时采集任务已重置");
        } catch (Exception e) {
            logger.error("服务器资源采集任务重置异常：" + e);
        }
    ...
    public String getMemoryRateForWindows() {
        ...
        try {
            ...
            logger.error("getThreadCountForWindows()报异常：" + e);
        }
    ...
    class SysInfoAcquirerTimerTask extends TimerTask {
        final String JVM_PID = "JVM_PID";
        final String THREAD_COUNT = "THREAD_COUNT";
        final String USE_MEMORY_RATE = "USE_MEMORY_RATE";
        final String CPU_RATE = "CPU_RATE";
        final String UP_SPEED = "UP_SPEED";
        final String IP = "IP";
        final String DOWN_SPEED = "DOWN_SPEED";
        final String FREE_MEMORY = "FREE_MEMORY";
        final String JVM_HEAP_INIT_MEMORY = "JVM_HEAP_INIT_MEMORY";
        final String JVM_HEAP_MAX_MEMORY = "JVM_HEAP_MAX_MEMORY";
        final String JVM_HEAP_USED_MEMORY = "JVM_HEAP_USED_MEMORY";
        final String JVM_MEMORY_RATE = "JVM_MEMORY_RATE";
        final String JVM_MEMORY_USE_TOTAL = "JVM_MEMORY_USE_TOTAL";
        final String MEMORY_USE_TOTAL = "MEMORY_USE_TOTAL";
        final String JVM_FREE_MEMORY = "JVM_FREE_MEMORY";
        int mb = 1048576;
        ...
        public void run() {
            try {
                logger.info("服务器资源采集任务开始：采集间隔(ms):" + PERIOD_TIME + "当前时间：" + new Date());
                long startTime = System.currentTimeMillis();
                int threadCount = getThreadCount();
                Double cpuRate = getSystemCpuLoad();//getCPURate(); // CPU使用率
                ...
                List<CommonKeyValue> models = new ArrayList<>();
...
//后续操作为将采集到的数据存放到数据库，可自行设计
                ...
                //总物理内存
                long total_memory = osmxb.getTotalPhysicalMemorySize() / mb;

                // logger.info("总物理内存："+total_memory+" MB");
                //剩余物理内存
                long free = osmxb.getFreePhysicalMemorySize() / mb;
                //  logger.info("物理剩余内存："+free+" MB");
                //已使用物理内存总-剩余
                long used_memory = total_memory - free;
                String use_total = used_memory + "/" + total_memory + "MB";
                //logger.info("已使用物理内存："+used_memory+" MB");
                //物理内存使用占比
                String usedRate = df.format(((double) used_memory / total_memory));
                // logger.info("已使用物理内存占比："+usedRate);
                //jvm内存计算
                //初始内存
                long jvm_total = Runtime.getRuntime().totalMemory() / mb;
//                logger.info("jvm占用内存："+jvm_total);
//                logger.info("jvm初始分配内存："+usage.getInit()/mb);
//                //最大可用内存
//
//                logger.info("jvm最大可用内存："+usage.getMax()/mb);
//                //已使用内存
//
//                logger.info("jvm已用内存："+usage.getUsed()/mb);
//                //剩余空间
//               logger.info("jvm剩余空间："+((double)jvm_total-((double)usage.getUsed()/mb)));
                //堆内存使用百分比
                String HeapMemoryRate = df.format(((double) usage.getUsed() / usage.getMax()));
//                logger.info("jvm堆内存已用占比："+HeapMemoryRate);
                String jvm_use_total = (usage.getUsed() / mb) + "/" + (usage.getMax() / mb) + "MB";
                double jvm_free = (double) jvm_total - ((double) usage.getUsed() / mb);
                String cpu_rate_str = df_two.format((double) cpuRate * 100);
                models.add(new CommonKeyValue(IP, CommonUtil.getServerIp()));
                models.add(new CommonKeyValue(JVM_PID, pid));
                models.add(new CommonKeyValue(THREAD_COUNT, String.valueOf(threadCount)));
                models.add(new CommonKeyValue(CPU_RATE, cpu_rate_str + "%"));
                models.add(new CommonKeyValue(UP_SPEED, upSpeed + "MB/s"));
                models.add(new CommonKeyValue(DOWN_SPEED, downSpeed + "MB/s"));
                models.add(new CommonKeyValue(FREE_MEMORY, free + "MB"));
                models.add(new CommonKeyValue(USE_MEMORY_RATE, usedRate));
                models.add(new CommonKeyValue(JVM_HEAP_INIT_MEMORY, usage.getInit() / mb + "MB"));
                models.add(new CommonKeyValue(JVM_HEAP_MAX_MEMORY, usage.getMax() / mb + "MB"));
                models.add(new CommonKeyValue(JVM_HEAP_USED_MEMORY, usage.getUsed() / mb + "MB"));
                models.add(new CommonKeyValue(JVM_MEMORY_RATE, HeapMemoryRate));
                models.add(new CommonKeyValue(JVM_MEMORY_USE_TOTAL, jvm_use_total));
                models.add(new CommonKeyValue(MEMORY_USE_TOTAL, use_total));
                models.add(new CommonKeyValue(JVM_FREE_MEMORY, (long) jvm_free + "MB"));

                long endTime = System.currentTimeMillis();
                prossorSaveMonitor.addValue("SystemInfoMonitorManagerKey", models);
                //logger.info("硬件数据采集完毕，已存到仓库"+"任务总耗时：" + (endTime - startTime)+" ms");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    @PreDestroy
    public void destroy() {
        logger.info("do nothing in @PreDestroy method");
    }
}